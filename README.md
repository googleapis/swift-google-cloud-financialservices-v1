...
Add `swift-google-cloud-financialservices-v1` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-financialservices-v1.git --from 0.0.0-preview
```

Then add `GoogleCloudFinancialServicesV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudFinancialServicesV1 <target-name> --package swift-google-cloud-financialservices-v1
```
